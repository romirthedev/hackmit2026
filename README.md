...
| Home dashboard (`/`) | A recording journal with original photos, a chronological list, focused recall, and reminders. Personal and caretaker views use the current recording, answer, rule, alert, people, and Notch context APIs. Draft answers update after evidence review. Signed-out and disconnected states are explicit; no synthetic memories are substituted. Full controls remain at `/workspace`, including Mac commands, people, context, pairing, and measured usage. |
