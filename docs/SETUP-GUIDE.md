# Full setup guide: ASUS backend, ESP32 camera, wearable microphone

This is the complete installation and acceptance manual that used to be the README. The short version, with the phone-first demo, is in the [project README](../README.md).
...
- [20-hour build and acceptance plan](20-HOUR-PLAN.md)
- [FORIOT / ESP32-CAM hardware and wireless setup](HARDWARE.md)
- [Architecture, storage, and model deployment](ARCHITECTURE.md)
- [Capture protocol and API](API.md)
...
- [Real YouTube video test: local 3B results and ASUS comparison procedure](evaluations/YOUTUBE-VIDEO-2026-09-19.md)
- [OpenCLIP/PyAV evaluation: zoo, first-person workday and held-out footage](evaluations/OPEN-VIDEO-2026-09-19.md)
...
| Home dashboard (`/`) | Card grid for the wearer with a caretaker view at `#care`: Ask (spoken, wake word on the phone), calendar with scanned bills and Notch reminders, Letters with scanned postcards, memory log, moments, people, questions. Signed-out and disconnected states are explicit; no synthetic memories are substituted. Full controls remain at `/workspace`. |
| Phone (`/phone`) | Record / Scan mail / talk. Continuous original video, durable retry queues, wake-word questions answered aloud with Deepgram, scanned mail filed to the home screen. Sign-in expiry stops camera/microphone and polling while preserving local originals for reconnection. |
...
For the primary phone experience, start with [phone and Notch setup](PHONE-NOTCH.md).
...
pass. The [20-hour plan](20-HOUR-PLAN.md) records the earlier hardware plan.
...
|---|---|
| One ESP32-CAM with OV2640 and PSRAM | The current firmware uses the AI-Thinker pinout. Check the PCB markings against [HARDWARE.md](HARDWARE.md). “FORIOT 3Pcs OV2640” alone does not uniquely identify the PCB. An OV2640 camera module alone is insufficient. |
...
The browser flows (voice, scanning, letter arrival) were checked by hand with headless Chrome; see [VOICE-AND-MAIL.md](VOICE-AND-MAIL.md) for what was measured.
